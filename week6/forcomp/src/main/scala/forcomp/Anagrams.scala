...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (for ((c,l) <- ((w.toLowerCase()).groupBy((c: Char) => c))) yield (c, l.size)).toList.sortWith(_._1 < _._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case Nil => Nil
    case _ => wordOccurrences(s.reduce((a, b) => a ++ b))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
	 def repeat(occ: (Char, Int)) = occ match {
	   case (c, num) => for (i <- 1 to num) yield (c, i)
	 }
	 ...
	 occurrences match {
    	...
    	case x :: xs => {
    	  ...
    	  (for { c <- combs; xreps <- repeat(x) } yield (xreps :: c)) ++ (for { c <- combs } yield (c))
    	}
	 }
  }
