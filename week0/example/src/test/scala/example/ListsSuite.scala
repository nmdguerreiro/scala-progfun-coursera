...
class ListsSuite extends FunSuite {
  ...
  test("sum of list with symmetric values is 0") {
    assert(sum(List(1, -1, 2, -2, 3, -3)) === 0)
  }
  
  test("sum of list with a 0 only is 0") {
    assert(sum(List(0)) === 0)
  }
  ...
  test("max on empty list throws NoSuchElementException") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  
  test("max supports repeated elements") {
    assert(max(List(1, 2, 3, 3, 2, 1)) === 3)
  }
  
  test("max supports negative elements") {
    assert(max(List(-1, -2, -3)) === -1)
  }
  
  test("max element at the beginning of the list") {
    assert(max(List(3, 2, 1)) === 3)
  }
  
  test("max element at the end of the list") {
    assert(max(List(1, 2, 3)) === 3)
  }
  
  test("max element in the middle of the list") {
